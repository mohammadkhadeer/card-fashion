package com.card_fashion.rest.view.Adapters;
...
import com.card_fashion.rest.model.ItemInDB;
import com.squareup.picasso.Picasso;

import java.util.ArrayList;

import static com.card_fashion.rest.functions.FunctionCart.calculateNumberOfItemsOnCart;
import static com.card_fashion.rest.functions.FunctionCart.deleteItemFromCartOnCart;
import static com.card_fashion.rest.functions.FunctionCart.deleteItemInCartFromDataBase;
...
//    List<Offer> mList = new ArrayList<>();
    ...
    private void actionListenerToAdd(final ViewHolder holder, final Context context, final int position) {
        holder.plusIV.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                calculateNumberOfItemsOnCart(context,sharedPreferences,editor,itemInDBArrayL.get(position),"add",Integer.parseInt(holder.numberOfItem.getText().toString()));
    ...
    private void actionListenerToRemove(final ViewHolder holder, final Context context, final int position) {
        holder.removeIV.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int x = Integer.parseInt(holder.numberOfItem.getText().toString()) -1;
                holder.numberOfItem.setText(String.valueOf(x));
                calculateNumberOfItemsOnCart(context,sharedPreferences,editor,itemInDBArrayL.get(position),"remove",Integer.parseInt(holder.numberOfItem.getText().toString()));
                passUpdate.onClicked(true);
                if (x == 0)
                {
    ...
    private void actionListenerToDelete(ViewHolder holder, final Context context, final int position) {
        holder.deleteButtonRL.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                deleteItemFromCartOnCart(context,sharedPreferences,editor,itemInDBArrayL.get(position).getNumberOfItemNow());
                deleteItemInCartFromDataBase(context,itemInDBArrayL.get(position).getName());
                removeItemFromList(position);
                passUpdate.onClicked(true);
            }
        });
